...
public class ChaosAddLifesCommand {
	public static int execute(ServerCommandSource context, Collection<ServerPlayerEntity> targets, int amount) throws CommandSyntaxException {
		...
		ChaosLifesComponents.PLAYER_LIFES.maybeGet(serverPlayer).ifPresent(lifesComponent -> {
			if (lifesComponent.getLifes() < lifesComponent.getMaxLifes()+1){
				serverPlayer.playSound(SoundEvents.BLOCK_NOTE_BLOCK_CHIME, SoundCategory.PLAYERS, 1.0F, 1.0F);
				context.sendFeedback(Text.translatable("commands.life.add.success", new Object[]{amount, ((ServerPlayerEntity)targets.iterator().next()).getDisplayName()}), true);
				...
				context.sendError(Text.translatable("commands.life.add.error", new Object[]{amount, ((ServerPlayerEntity) targets.iterator().next()).getDisplayName(), lifesComponent.getMaxLifes()}));
			}
